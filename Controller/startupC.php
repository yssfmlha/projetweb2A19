...
class startupC{
    ...
    public function addstartup($startup){
        $sql="INSERT INTO startup2 VALUE (NULL,:iduser,:Nom,:domaine,:nom,:prenom,:de,:email,:tel)";
        $db=config::getConnexion();
        try{
            $query=$db->prepare($sql);
            $query->execute(["iduser"=>$startup->getid_user(),"Nom"=>$startup->getNom(),"domaine"=>$startup->getDomaine(),"nom"=>$startup->getNom_f(),
    ...
    public function SearchStartupByUser($id)
    {
            try
            {
                $db = Config::getConnexion();
                $stmt = $db->prepare('SELECT * FROM startup2 WHERE id_user ='.$id);
                $stmt->execute();
                return $stmt->fetchAll(PDO::FETCH_ASSOC);
            }
            catch(Exception $e)
